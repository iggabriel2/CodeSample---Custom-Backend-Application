...
{
    public class GetKeywordsSnapshot
    {
        public async Task<List<KeywordSnapshot>> GetSnapshot(APIAuthorizationRequest request, ClientProfileCodes profileCode, string snapshotUrl)
        {
            try
            {
                ...
                if (responseMessage.IsSuccessStatusCode)
                {
                    ...
                    List<KeywordSnapshot> getValues = JsonConvert.DeserializeObject<List<KeywordSnapshot>>(rawReportOutput);
                    return getValues;
                }
                else
                {
            ...
            {
                await ErrorLogging.LogError(ex.ToString(), "GetSnapshot - GetKeywordsSnapshot", JsonConvert.SerializeObject(request) + " " + JsonConvert.SerializeObject(profileCode) + " " + snapshotUrl + " ", request.ClientId);
                return null;
            }
        }
    }
