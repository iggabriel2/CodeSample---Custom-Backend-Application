...
{
    public class GetAdGroupsSnapshot
    {
        public async Task<List<AdGroupSnapshot>> GetSnapshot(APIAuthorizationRequest request, ClientProfileCodes profileCode, string snapshotUrl)
        {
            try
            {
                ...
                if (responseMessage.IsSuccessStatusCode)
                {
                    ...
                    List<AdGroupSnapshot> getValues = JsonConvert.DeserializeObject<List<AdGroupSnapshot>>(rawReportOutput);
                    return getValues;
                }
                else
                {
            ...
            {
                await ErrorLogging.LogError(ex.ToString(), "GetSnapshot - GetAdGroupsSnapshot", JsonConvert.SerializeObject(request) + " " + JsonConvert.SerializeObject(profileCode) + " " + snapshotUrl + " ", request.ClientId);
                return null;
            }
        }
    }
